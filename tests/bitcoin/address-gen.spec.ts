...
describe('Bitcoin key generation', () => {
  ...
  describe('Sanity check tests copied from `bitcoinjs-lib` vs other libs', () => {
    test.skip('can create a BIP49, bitcoin testnet, account 0, external address', async () => {
      const mnemonic =
      ...
      if (!privateKey) throw new Error('Keychain must not be null');

      expect(root.privateKey?.toString('hex')).toEqual(Buffer.from(privateKey).toString('hex'));
      ...
      console.log('xxxx', bitcoinPayment.address);
      console.log('xxxx', bitcoinPayment.witness);
      console.log('xxxx', bitcoinPayment.redeem);

      expect(bitcoinPayment.address).toEqual('2Mww8dCYPUpKHofjgcXcBCEGmniw9CoaiD2');

      const child2 = keychain.derive(path);
      // console.log(child2);
      if (!child2.publicKey) throw new Error('no pubkey');

      const x = base58check(sha256).encode(child2.publicKey);
      console.log(x);
    });
  });
  ...
  // test.skip('against sparrow wallet', async () => {
  //   const seed = await deriveBtcBip49SeedFromMnemonic(phrase);

  //   // https://iancoleman.io/bip39/
  //   expect(Buffer.from(seed).toString('hex')).toEqual(
  //     '743d55f356ea12afbcbc9717993b9a76f8d67e0af7ec69e2a47419c279c107c66e28e7995e2417b9330cd77b4462ef6ce40776a6ae5240414fe6367fc4f23357'
  //   );

  //   const bip32seed = bip39.mnemonicToSeedSync(phrase);
  //   expect(bip32seed.toString('hex')).toEqual(
  //     '743d55f356ea12afbcbc9717993b9a76f8d67e0af7ec69e2a47419c279c107c66e28e7995e2417b9330cd77b4462ef6ce40776a6ae5240414fe6367fc4f23357'
  //   );

  //   const rootFromScureBip32 = deriveRootBtcKeychain(seed);

  //   console.log('sdfsd', rootFromScureBip32.publicExtendedKey);

  //   expect(rootFromScureBip32.publicExtendedKey).toEqual(
  //     'xpub6DEyndt6iXQ72xzmwMpnsxyfYc1o4rBFFFXNR2tPMnhcQ5jk28NCQB9rahWQCxmHot684BNZsqPaqQ3nYN2GRboxTsHuCtaFoWVkrwzEVvu'
  //   );

  //   const rootFromBip32lib = '';

  //   const path = "m/49'/0'/0'";
  //   const child = rootFromScureBip32.derive(path);

  //   const { address } = await deriveBtcPayment(child.publicKey!);
  //   expect(address).toEqual('32zHvPvmiLRQqEtz63j43atW7sFHDnbKAV');
  // });

  // test.skip('against sparrow wallet testnet', async () => {
  //   const seed = await deriveBtcBip49SeedFromMnemonic(phrase);
  ...
  //   const payment = await deriveBtcPayment(child.publicKey!);
  //   // expect(payment)
  //   // expect(address).toEqual('32zHvPvmiLRQqEtz63j43atW7sFHDnbKAV');
  // });

  describe('BIP-49 logic', () => {
